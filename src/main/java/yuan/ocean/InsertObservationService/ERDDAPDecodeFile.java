...
public class ERDDAPDecodeFile implements IDecodeFile {
    ...
    public void decode(Map<String, String> linkedProperty, String paltCode, Station station,String subFilePath) {
        ...
        if (file.exists()) {
            try {
                ...
                int latPos = Integer.valueOf(linkedProperty.get("lat"));
                int lonPos = Integer.valueOf(linkedProperty.get("lon"));
                            ...
                            for (ObservedProperty property : sensor.getObservedProperties()) {
                                if (!property.getPropertyID().equals("urn:ogc:def:phenomenon:OGC:1.0.30:position")) {
                                   ...
                                   // System.out.println(property.getPropertyID());
                                    if (!eles[Integer.valueOf(linkedProperty.get(property.getPropertyID()))].equals("NaN"))
                                        property.tempSumValue = property.tempSumValue + Double.valueOf(eles[Integer.valueOf(linkedProperty.get(property.getPropertyID()))]);
                                    property.count++;
                                } else {
